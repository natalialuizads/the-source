...
export function TheSource() {
  const cards = [
    {
      title: 'Fundamentos',
      description: 'Algoritmos, Estruturas de Dados e conceitos base da computação.',
      href: '/01-fundamentos',
      icon: BookOpen,
      color: 'text-blue-400',
      bg: 'bg-blue-400/10',
    ...
    <div className="flex flex-col gap-12 py-8">
      <div className="flex flex-col gap-4">
        ...
        <p className="text-lg text-muted-foreground max-w-2xl">
          Seu segundo cérebro para desenvolvimento de software. 
          Uma base de conhecimento centralizada para fundamentos, ferramentas e referências rápidas.
        </p>
      </div>
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {cards.map((card) => (
          <Link
            key={card.href}
            href={card.href}
            className="group relative flex flex-col gap-4 overflow-hidden rounded-xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 transition-colors"
          >
            ...
            <div className="flex flex-col gap-2">
              <h3 className="font-semibold text-xl text-white group-hover:text-neon-green transition-colors">
                {card.title}
              </h3>
